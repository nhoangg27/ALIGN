...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    with torch.no_grad():
        ...
        y_pred = (np.asarray(y_pred, dtype=np.float64)) * 1000 - 300

        y_true = (np.asarray(y_true, dtype=np.float64)) * 1000 - 300

        std = (np.asarray(stdL, dtype=np.float64)) * 1000 - 300
    ...
    if use_pretrained:
        ...
        if args.metric == "auc":
            ...
            median_rel_error = np.median(e_rel)
            mean_rel_error = np.mean(e_rel)
            logger.info(f"mae: {mae:.2f}")
            logger.info(f"rmse: {rmse:.2f}")
            logger.info(f"median relative error: {median_rel_error:.2f}%")
            logger.info(f"mean relative error: {mean_rel_error:.2f}%")
            save = args.save_path
            ...
            if save != 'None':
                ...
                with open(save, 'w', newline='') as file:
                    ...
                    writer.writerow(["SMILES", "Method", 'True CV', "Predicted CV", 'STD'])
                    for row in stack:
                        writer.writerow(row)
