...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    with torch.no_grad():
        ...
        y_pred = np.asarray(y_pred, dtype=np.float64) * 10000
        y_true = np.asarray(y_true, dtype=np.float64) * 10000
        std = (np.asarray(stdL, dtype=np.float64)) * 10000 
