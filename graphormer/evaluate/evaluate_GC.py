...
def eval(args, use_pretrained, checkpoint_path=None, logger=None):
    ...
    with torch.no_grad():
        ...
        y_pred = np.asarray(y_pred, dtype=np.float64) * 100000
        y_true = np.asarray(y_true, dtype=np.float64) * 100000
        std = (np.asarray(stdL, dtype=np.float64)) * 100000
    ...
    if use_pretrained:
        ...
        if args.metric == "auc":
            ...
            logger.info(f"50th percentile: {p50:.2f}")
            ...
            if save != 'None':
                ...
                with open(save, 'w', newline='') as file:
                    ...
                    writer.writerow(["SMILES", "Method", 'True RI', "Predicted RI", 'STD'])
                    for row in stack:
                        writer.writerow(row)
