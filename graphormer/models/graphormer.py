...
from fairseq import models as fmodels
from fairseq import modules as fmodules
import sys
import gc
...
@fmodels.register_model("graphormer")
class GraphormerModel(fmodels.FairseqEncoderModel):
    def __init__(self, args, encoder):
        super().__init__(encoder)
        self.args = args

        # if getattr(args, "apply_graphormer_init", False):
        #     self.encoder.graph_encoder.apply(init_graphormer_params)
    ...
    def build_model(cls, args, task):
        ...
        if not utils.safe_hasattr(args, "max_nodes"):
...
class GraphormerEncoder(fmodels.FairseqEncoder):
    def __init__(self, args):
        ...
        if self.isconv:
            ...
            self.embed_out.bias.data = torch.ones(args.num_classes) * 0.005
            self.embed_out.bias.requires_grad = True

            self.mu_out.bias.data = torch.ones(args.num_classes) * 0.005
    ...
    def forward(self, batched_data, perturb=None, masked_tokens=None, **unused):
        ...
        if umap:
            ...
            graph_token = np.asarray(graph_token.cpu().detach())
            sm = np.asarray(sm)    
            stack = np.concatenate((sm, graph_token), axis=1)
            label = batched_data['idx'][0].cpu().detach().numpy()
            rootdir = '../../umap/'
            os.makedirs(rootdir, exist_ok=True)
            with open(rootdir + f'batched_embeddings_{label}.pkl', 'wb') as f:
        ...
        y = self.mu_out(y)
        y = F.relu(y) 
        x = F.relu(x) #APRIL 9 2025 PUT THIS BACK for non DMS
...
@fmodels.register_model_architecture("graphormer", "graphormer")
...
@fmodels.register_model_architecture("graphormer", "graphormer_base")
...
@fmodels.register_model_architecture("graphormer", "graphormer_slim")
...
@fmodels.register_model_architecture("graphormer", "graphormer_large")
