...
class SID(FairseqCriterion):
    ...
    def forward(self, model, sample, reduce=True):
        ...
        mu = mu.squeeze()
        std = std.squeeze()
        # for pred, label in zip(mu, label):
        #     print(pred, label)
        # exit()
        label=label.squeeze()
        # print(label[:5])
        ...
        # print(label[:10] * 1000 - 100)
        # print(mu[:10] * 1000 - 100)
        # exit()
        loss = self.log_loss(mu, std, label) * 100 #APRIL 9 2025 PUT THIS BACK
        # print(loss, mu, std, label)
        # exit()
        logging_output = {
            "loss": loss,
            "sample_size": 1,
